...
def get_defos3(dim, gf_d, gf_r):
    gf_d.vec.SetParallelStatus(gf_r.vec.GetParallelStatus())
    vd = gf_d.vec.local_vec
    vr = gf_r.vec.local_vec
    dim_ext = dim + dim * (dim-1) // 2
    for k in range(len(vd)//dim):
        vd[k] = vr[k]

def get_defos4(dim, gf_d, gf_r):
    gf_d.vec.SetParallelStatus(gf_r.vec.GetParallelStatus())
    vd = gf_d.vec.local_vec
    vr = gf_r.vec.local_vec
    dim_ext = dim + dim * (dim-1) // 2
    for k in range(len(vd)//dim):
        for l in range(dim):
            vd[k*dim+l] = vr[k*dim_ext+l]

def dummy():
    pass
            
def shape_test(mesh, maxh, V, a, pc, multidim, Vdim, order=1, reo=False):
    dim = mesh.dim
    ...
    if Vdim == dim:
        ...
        if Vdim != 1:
            ...
            update_vecs = lambda : get_defos3(dim, defo_bf, test_bf)
        else:
            update_vecs = lambda : get_defos3(dim, defo_bf, test_bf)
