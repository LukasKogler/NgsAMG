...
namespace amg
{
  ...
  using T_MESH           = H1AMGFactory<3>::TMESH;
  using T_ENERGY         = H1AMGFactory<3>::ENERGY;
  ...
  // not compiling elmat for 3d
  using PCCBASE = VertexAMGPC<H1AMGFactory<3>>;
  // using PCC = ElmatVAMG<H1AMGFactory<3>, double, double>;

  // RegisterPreconditioner<PCC> register_h1amg_1d ("ATAmg.h1_scal");
  RegisterAMGSolver<PCCBASE> register_h1amg_3d ("h1_3d");
