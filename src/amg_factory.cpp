...
namespace amg
{
  ...
  shared_ptr<BaseDOFMapStep> BaseAMGFactory :: DoStep (shared_ptr<AMGLevel> & f_lev, shared_ptr<AMGLevel> & c_lev, State & state)
  {
    ...
    const bool doco = (fcomm.Rank() == 0) && (options->log_level != Options::LOG_LEVEL::NONE);
  ...
  bool BaseAMGFactory :: TryCoarseStep (State & state)
  {
    ...
    if ( (cmap->GetMesh()->GetEQCHierarchy()->GetCommunicator().Rank() == 0) && (options->log_level != Options::LOG_LEVEL::NONE) )
