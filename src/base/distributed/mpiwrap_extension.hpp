...
namespace amg
{
  ...
  public:
    ...
    template <typename T, typename T2 = decltype(GetMPIType<T>())>
    void AllReduceFA (FlatArray<T> d, const MPI_Op op = MPI_SUM) const
    {
      if (!valid_comm)
        { return; }
      MPI_Allreduce ( MPI_IN_PLACE, d.Data(), int(d.Size()), GetMPIType<T>(), op, comm);
    }

    NgsAMG_Comm CreateSubCommunicatorGlobal (FlatArray<int> procs) const
    {
      /**
       *  Unlike NgMPI_Comm::SubCommunicator, this uses MPI_Comm_create
       *  instead of MPI_Comm_create_group - i.e., this must be called by ALL
      ...
      MPI_Comm_create(comm, gsubcomm, &subcomm);
      return (subcomm == MPI_COMM_NULL) ? NgsAMG_Comm() : NgsAMG_Comm(subcomm, true);
    }
  }; // class NgsAMG_Comm
  ...
  INLINE void ExchangePairWise (NgsAMG_Comm comm, FlatArray<int> dps, const TIN& send_data, TOUT & recv_data)
  {
    if (!comm.isValid())
      { return; }
    Array<MPI_Request> reqs(2 * dps.Size());
    for (auto k : Range(dps)) {
      reqs[2*k]     = comm.ISend(send_data[k], dps[k], MPI_TAG_AMG);
      reqs[2*k + 1] = comm.IRecv(recv_data[k], dps[k], MPI_TAG_AMG);
    }
    MyMPI_WaitAll(reqs);
  } // SendRecvData
  ...
  INLINE void ExchangePairWise2 (NgsAMG_Comm comm, FlatArray<int> dps, const TIN& send_data, TOUT & recv_data)
  {
    ...
    for (auto k : Range(dps))
      { reqs[k]     = comm.ISend(send_data[k], dps[k], MPI_TAG_AMG); }
    for (auto k : Range(dps))
      { comm.Recv(recv_data[k], dps[k], MPI_TAG_AMG); }
    MyMPI_WaitAll(reqs);
  } // SendRecvData
