...
namespace ngcore
{
  template<> struct MPI_typetrait<amg::NCStokesGGED<2>> {
    static NG_MPI_Datatype MPIType () {
      ...
      {
        int block_len[2] = { 2, 1 };
        NG_MPI_Aint displs[2] = { 0, 2 * sizeof(amg::NCStokesGGED<2>::TED) };
        NG_MPI_Datatype types[2] = { GetMPIType<amg::NCStokesGGED<2>::TED>(), GetMPIType<amg::NCStokesGGED<2>::TFLOW>() };
  ...
  template<> struct MPI_typetrait<amg::NCStokesGGED<3>> {
    static NG_MPI_Datatype MPIType () {
      ...
      {
        int block_len[2] = { 2, 1 };
        NG_MPI_Aint displs[2] = { 0, 2 * sizeof(amg::NCStokesGGED<3>::TED) };
        NG_MPI_Datatype types[2] = { GetMPIType<amg::NCStokesGGED<3>::TED>(), GetMPIType<amg::NCStokesGGED<3>::TFLOW>() };
        NG_MPI_Type_create_struct(2, block_len, displs, types, &NG_MPI_T);
        NG_MPI_Type_commit ( &NG_MPI_T );
      }
      return NG_MPI_T;
